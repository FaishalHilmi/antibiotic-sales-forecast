...
export default function DetailKasirView() {
  ...
  const [name, setName] = useState<string>(dummyDataCashier[0].name);
  const [username, setUsername] = useState<string>(
    dummyDataCashier[0].username
  );
  const [password, setPassword] = useState<string>(
    dummyDataCashier[0].password
  );
  const [role, setRole] = useState<string>(dummyDataCashier[0].role);
